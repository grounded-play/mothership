...
export default function AppFrame({ children }: { children: React.ReactNode }) {
    ...
    const [compactPanel, setCompactPanel] = useState<"player" | "guide" | null>(null);
                                ...
                                isCompactViewport ? (
                                    <div className="relative flex items-center justify-center px-3 pb-3">
                                        <div className="flex items-center justify-center gap-2">
                                            <button
                                                type="button"
                                                onClick={() => setCompactPanel((current) => current === "player" ? null : "player")}
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setCompactPanel((current) => current === "guide" ? null : "guide")}
                                                className={`flex items-center gap-2 rounded-full border px-3 py-2 text-[10px] font-bold uppercase tracking-[0.22em] shadow-[0_8px_24px_rgba(0,0,0,0.4)] transition-colors ${
                                                    compactPanel === "guide"
                                                        ? "border-neon-cyan bg-cyan-500/15 text-neon-cyan"
                                                        : "border-white/10 bg-black/85 text-white"
                                        ...
                                        {compactPanel && (
                                            <div className="pointer-events-auto absolute bottom-[calc(100%+0.75rem)] left-1/2 z-[90] w-[min(100%,28rem)] -translate-x-1/2">
